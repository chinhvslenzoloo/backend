...
import MergedDashboardSection from './components/dashboards/user/MergedDashboardSection'
...
function App() {
  ...
  useEffect(() => {
    // Attempt to load user data on mount (relying on refresh interceptor if token is missing/expired)
    if (!auth.user) {
      auth.loadMe().catch(() => {})
    }
  }, [])
