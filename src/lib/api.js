...
const baseURL =
  ...
  "https://frontend-production-fc88.up.railway.app/api";
...
export const api = {
  ...
  async driverUpdateStatus(orderId, status) {
    const res = await client.post(`/driver/orders/${orderId}/status`, {
      status,
    });
    return res.data;
  },
  ...
  async adminAssignOrder(orderId, driverId) {
    const res = await client.post(`/admin/orders/${orderId}/assign`, {
      driverId,
    });
    return res.data;
  },
